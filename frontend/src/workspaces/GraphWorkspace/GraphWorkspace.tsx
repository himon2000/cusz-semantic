...
function searchResultKey(result: SearchResult): string {
  return result.relationship ? `relationship:${result.relationship.edge.id}` : `node:${result.node.id}`;
}

function searchResultLabel(result: SearchResult): string {
  return result.relationship?.label || result.node.content || result.node.id;
}

function searchResultType(result: SearchResult): string {
  return result.relationship?.edge.type || result.node.type;
}
...
const ENTITY_VISUAL_KEY: Array<{ shape: GraphEntityShapeVariant; label: string }> = [
  { shape: "community", label: "University" },
  { shape: "entity", label: "School" },
  { shape: "biomolecule", label: "Person" },
  { shape: "process", label: "Program" },
  { shape: "condition", label: "AcademicField" },
  { shape: "compound", label: "ResearchArea" },
];
      ...
      {suggestionsOpen && suggestions.length > 0 ? (
        <ul id={listboxId} role="listbox" className="explore-search-suggestions" aria-label="Search suggestions">
          {suggestions.map((result, index) => (
            ...
            >
              <span className="explore-search-suggestion-label">{searchResultLabel(result)}</span>
              <span className="explore-search-suggestion-type">
                {result.relationship ? <><span>Relationship</span><span aria-hidden="true"> · </span></> : null}
...
export function GraphWorkspace({ externalFocusNodeId, externalFocusToken }: GraphWorkspaceProps = {}) {
  ...
  const selectSearchResult = useCallback((result: SearchResult) => {
    ...
    if (!relationship) {
      requestViewMode("full");
      focusNode(result.node.id);
      window.setTimeout(() => sceneRef.current?.focusNode(result.node.id), 0);
      return;
    }

    const compactQuery = searchQuery.replace(/\s+/g, "").toLocaleLowerCase();
    const sourceMatches = relationship.source_node.content.replace(/\s+/g, "").toLocaleLowerCase().includes(compactQuery);
    const targetMatches = relationship.target_node.content.replace(/\s+/g, "").toLocaleLowerCase().includes(compactQuery);
    const focusNodeId = targetMatches && !sourceMatches
      ? relationship.target_node.id
      : relationship.source_node.id;

    requestViewMode("full");
    setFocusedNodeId("");
    setSelectedNodeId(focusNodeId);
    setSelectedEdgeId(relationship.edge.id);
    setPathResult(null);
    setSearchResults([]);
    setSearchError("");
    window.setTimeout(() => sceneRef.current?.focusNode(focusNodeId), 0);
  }, [focusNode, requestViewMode, searchQuery]);
  ...
  const pluginToolbarItems = useMemo<GraphPluginToolbarItem[]>(
    () => pluginRegistry
      .filter((entry) => entry.id !== "temporal-overlay" || Boolean(temporalBounds?.min && temporalBounds?.max))
      .map((entry) => ({
        id: `${entry.id}-toggle`,
        label: entry.label,
        title: entry.title,
    ...
    [pluginContext, pluginRegistry, temporalBounds?.max, temporalBounds?.min],
  );
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  <div className="explore-search-results hud-scrollbar" style={searchResultsStripStyle}>
                    {searchResults.map((result) => (
                      <button key={searchResultKey(result)} style={predictionCardStyle} onClick={() => selectSearchResult(result)}>
                        <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                          <div style={{ minWidth: 0 }}>
                            <div style={{ color: "#fff", fontWeight: 600 }}>{searchResultLabel(result)}</div>
