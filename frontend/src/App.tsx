...
import { installInterfaceLanguage, type InterfaceLanguage } from './i18n';
...
const KnowledgeEngineeringWorkspace = lazy(() => import('./workspaces/KnowledgeEngineeringWorkspace/KnowledgeEngineeringWorkspace').then((module) => ({ default: module.KnowledgeEngineeringWorkspace })));
const OntologyWorkspace = lazy(() => import('./workspaces/OntologyWorkspace').then((module) => ({ default: module.OntologyWorkspace })));

type WorkspaceId = 'welcome' | 'explore' | 'analyze' | 'decisions' | 'enrich' | 'manage' | 'ontology-hub' | 'knowledge-engineering';
...
const navItems: NavItem[] = [
  ...
  { id: 'knowledge-engineering', label: '学院知识图谱', hint: '导入官网资料、浏览人物关系并向图谱提问', icon: Workflow },
];
  ...
  return (
    <main className="landing-page">
      <div className="landing-shell">
        ...
        <section className="landing-hero">
          <div className="landing-copy">
            <div className="landing-status-bar" data-status={stats.status}>
              ...
              <span className="landing-status-version">Semantica v2 · 本地知识图谱</span>
            </div>
            ...
            <div className="landing-cta-row">
              ...
              <button className="landing-cta-secondary" type="button" onClick={onOpenKnowledgeGraph}>
          ...
          <div className="landing-preview" aria-label="Knowledge graph preview">
            ...
            <div className="landing-command-card">
              ...
              <div>
                <div className="landing-command-label">搜索教师、项目或研究主题</div>
                <div className="landing-command-meta">按关系远近查看 · 聚焦条目 · 查找联系路线</div>
              </div>
            </div>
            ...
            <div className="landing-dossier-card">
              <div className="landing-dossier-kicker">知识条目示例</div>
              <div className="landing-dossier-title">数据科学学院</div>
              <div className="landing-dossier-row"><span>教师档案</span><strong>105</strong></div>
              <div className="landing-dossier-row"><span>培养项目</span><strong>12</strong></div>
              <div className="landing-dossier-row"><span>官网来源</span><strong>可追溯</strong></div>
            </div>
            <div className="landing-timeline-card">
              <div className="landing-timeline-header">
                <span className="landing-timeline-title">官网数据快照</span>
                <span className="landing-timeline-badge">2026-09-05</span>
              </div>
        ...
        {/* ── Live metrics ── */}
        <div className="landing-metrics" aria-label="系统状态">
          {metrics.map((metric) => (
        ...
        {/* ── Capability band ── */}
        <section className="landing-capability-band" aria-label="知识图谱能力">
          <div className="landing-capability-label">关系分析</div>
          <div className="landing-capability"><Radar size={12} />查看关系远近</div>
          <div className="landing-capability"><Network size={12} />聚焦相关条目</div>
          <div className="landing-capability"><GitMerge size={12} />发现关系小组</div>
          <div className="landing-capability"><Route size={12} />查找联系路线</div>
          <div className="landing-capability"><ShieldCheck size={12} />查看信息来源</div>
        </section>
...
export default function App() {
  const [language, setLanguage] = useState<InterfaceLanguage>(() => {
    return window.localStorage.getItem('semantica-interface-language') === 'en' ? 'en' : 'zh-CN';
  });
  ...
  useEffect(() => {
    window.localStorage.setItem('semantica-interface-language', language);
    return installInterfaceLanguage(language);
  }, [language]);
  ...
  const renderWorkspace = () => {
    if (activeWorkspace === 'welcome') {
      return (
        <WelcomeScreen
          ...
          onOpenKnowledgeGraph={() => setActiveWorkspace('knowledge-engineering')}
          onOpenVocabulary={() => {
            setActiveWorkspace('explore');
    ...
    if (activeWorkspace === 'knowledge-engineering') {
      return (
        <WorkspaceShell
          title="港中深数据科学学院知识图谱"
          subtitle="导入官网资料，整理教师、项目和研究主题，浏览关系并向本地图谱提问。"
          kicker="SDS 官网知识图谱"
          compact
        >
          <ErrorBoundary key="knowledge-engineering">
            <Suspense fallback={<WorkspaceFallback />}>
              <KnowledgeEngineeringWorkspace
